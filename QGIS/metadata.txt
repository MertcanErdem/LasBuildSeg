...
# ---------------------------------------------------------------------------
# ---------------------------------------------------------------------------
# Presets calibrated from the S1_alpha parameter sweeps over four test sites
# (Purdue, Hicks Dome, England, France). For each (alpha, kernel_size) cell the
# metrics were averaged across all four sites and the best cell chosen per goal:
#
#   Preset        alpha  kernel   mean P   mean R   mean F1  mean IoU
#   ------------  -----  ------   ------   ------   -------  --------
#   Aggressive     0.10     3      0.795    0.845    0.811    0.696   (max recall)
#   Mild           0.20     3      0.923    0.808    0.860    0.762   (best F1 & IoU)
#   Conservative   0.40     7      0.960    0.681    0.772    0.666   (max precision)
#
# CALIBRATED from the sweep:  alpha, kernel_size.
# VALIDATED but constant:     squareness = 0.30  (the only value tested in the
#                             sweep — every row used 0.3, so it stays fixed).
# NOT in the sweep (heuristic gradient, NOT data-derived): min_size, max_size,
#                             width. Tune later if you sweep them.
#
# The FP-elimination filters and per-component alpha shape are OFF in all three
# presets on purpose: the metrics above were measured with that pipeline config
# (matching the standalone). Turning them on would change the measured P/R/F1,
# so they are left for manual opt-in rather than baked into a preset.
#
# Interpretation: Aggressive = aggressive DETECTION (loose, max recall);
# Conservative = cautious (strict, max precision); Mild = best-balanced default.
# ---------------------------------------------------------------------------
PRESETS = {
    "Mild": {
        "alpha":               0.20,   # data-derived (best F1 & IoU across sites)
        "min_size":            100,    # heuristic (not swept)
        "max_size":            50000,  # heuristic (not swept)
        "squareness":          0.30,   # only value tested in sweep
        "width":               3.0,    # heuristic (not swept)
        "kernel_size":         3,      # data-derived
        "use_height_var":      False,
    ...
    "Conservative": {
        "alpha":               0.40,   # data-derived (max precision, ~0.96 mean)
        "min_size":            150,    # heuristic (not swept)
        "max_size":            50000,  # heuristic (not swept)
        "squareness":          0.30,   # only value tested in sweep
        "width":               4.0,    # heuristic (not swept)
        "kernel_size":         7,      # data-derived
        "use_height_var":      False,  # left off so measured precision holds
    ...
    "Aggressive": {
        "alpha":               0.10,   # data-derived (max recall, ~0.85 mean)
        "min_size":            50,     # heuristic (not swept)
        "max_size":            100000, # heuristic (not swept)
        "squareness":          0.30,   # only value tested in sweep
        "width":               2.0,    # heuristic (not swept)
        "kernel_size":         3,      # data-derived
        "use_height_var":      False,
...
class LasBuildSegDialog(QDialog):
    ...
    Full-featured dialog for LasBuildSeg with:
      ...
      - Quick presets: Mild / Conservative / Aggressive
    ...
    def __init__(self, parent=None):
        ...
        self._apply_preset_by_name("Mild")
